...
main () 
{
  ...
  polynomial * create;
	...
	term_t t ;
	t.coef = 2 ; t.expo = 0 ;	polynomial_add_term(p1, &t) ; 
	t.coef = 3 ; t.expo = 3 ; 	polynomial_add_term(p1, &t) ;
	t.coef = 2 ; t.expo = 2 ;	polynomial_add_term(p1, &t) ;

	t.coef = 8 ; t.expo = 2 ; 	polynomial_add_term(p2, &t) ;
	t.coef = 2 ; t.expo = 3 ;	polynomial_add_term(p2, &t) ;
	t.coef = 10 ; t.expo = 1 ;	polynomial_add_term(p2, &t) ;

	//pr = polynomial_add(p1, p2) ;
  sub = polynomial_subt(p1, p2);

	polynomial_print(p1) ;
	polynomial_print(p2) ;
  polynomial_print(sub);
  ...
  scanf("%255[^\n]", buf);
  scanf("%c", &newline);
  create = polynomial_create(buf);
  polynomial_print(create);
